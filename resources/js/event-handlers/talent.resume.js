...
handler.prototype.refresh = function () {
  ...
  self.core.resource.talent.get(data)
    ...
    .then(function (res) {
      ...
      let photos = _.filter(self.talent.bam_talent_media2, function (val) {
        return val.type == 1
      })
      if (photos[0]) {
        let firstphoto = _.slice(photos, 0, 1)
        self.core.service.databind('#first-photo', firstphoto[0])
      }

      if (photos[1]) {
        let secondphoto = _.slice(photos, 1, 2)
        self.core.service.databind('#second-photo', secondphoto[0])
      }

      if (photos[2]) {
        let thirdphoto = _.slice(photos, 2, 3)
