...
handler.prototype.refreshRoles = function(e) {
	...
	if (e) {
		var url = '/messages/' + self.projectId;
...
handler.prototype.refreshConversations = function(e) {
	...
	if (e) {
		var url = '/messages/' + self.projectId + '/' + self.roleId;
		window.history.pushState(null, null, url);
	}
	...
	if (self.roleId) {
		...
		var role_ids = _.map(self.project.bam_roles, function(role) {
			return role.role_id;
		});
...
handler.prototype.refreshMessages = function() {
	...
	var schedule = _.find(self.schedules.data, function(schedule) {
...
handler.prototype.sendMessage = function() {
	if ($('#message-text').val()) {
		...
		self.core.resource.message.post(data)
			.then(function(res) {
				var $element = $('[data-bind-template].conversation-box').clone();
				var data2 = res;
