...
handler.prototype.refresh = function () {
  ...
  promise.then(function () {
    if (self.campaign) { return self.getSchedules() } else { return $.when() }
  })
    .then(function (res) {
      if (res) {
...
handler.prototype.getSchedules = function () {
  ...
  let form = self.core.service.form.serializeObject('#filter-form')
  ...
  return self.core.resource.search_talent.get(data)
    .then(function (res) {
      talents = res
      let schedule_ids = _.map(res.data, function (talent) {
...
handler.prototype.reschedule = function (e) {
  ...
  self.core.resource.schedule.patch(data)
    ...
    .then(function (res) {
      let conversation = _.first(res.data)
...
handler.prototype.showMessageModal = function (e) {
  ...
  let scheduleId = $element.parents('.schedule').attr('data-id')
  ...
  self.core.resource.conversation.get(data)
    ...
    .then(function (res) {
      ...
      _.each(conversation.messages, function (val, ind) {
        ...
        let path = val.body.match(regex)

        if (path && path.length >= 8 && path[7]) {
          let embed = '<br/><iframe style="border:0px" width="420" height="315" src="https://www.youtube.com/embed/' + path[7] + '"> </iframe>'
          val.body += embed
        }
      })
