...
jscore.run(function (core) {
  ...
  if (qs.access_token) {
    ...
    localStorage.setItem(
      "expires_on",
      Math.round(new Date().getTime() / 1000) + parseInt(qs.expires_in)
    );
  ...
  function init(user) {
      ...
      401: function () {
        if (
          ...
          window.location.href =
            "/login?redirect=" + encodeURIComponent(window.location);
        }
      },
    };
    ...
    core.service.router.$$controllers = require("./controllers/**/*.js", {
      hash: true,
    });
    ...
    // Check which version of the skin we want?
    if (/(.*)auditions\.com$/.test(window.location.hostname)) {
    ...
    core.service.router
      ...
      .add("/alternate-emails", "alternate-emails")
      .add("/welcome", "welcome")
      ...
      .add(
        "/projects/{projectId}/roles/{roleId}/like-it-list",
        "role.like-it-list"
      )
      .add(
        "/projects/{projectId}/roles/{roleId}/find-talents",
        "role.find-talents"
      )
      .add(
        "/projects/{projectId}/roles/{roleId}/submissions",
        "role.submissions"
      )
      ...
      .add(
        "/projects/{projectId}/roles/{roleId}/public-like-it-list",
        "role.publiclikeitlist"
      )
      .add(
        "/projects/{projectId}/roles/{roleId}/public-like-it-list/{accessToken}",
        "role.publiclikeitlist"
      )
